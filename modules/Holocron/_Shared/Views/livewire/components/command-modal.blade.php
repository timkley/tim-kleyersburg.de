    ...
    <flux:modal name="command-modal"
        ...
        <flux:command.no-clear :filter="false">
            ...
            @foreach($labels as $type => $label)
                <flux:text class="px-4 py-2 bg-white dark:bg-zinc-700 flex items-center gap-x-1">
                    <flux:icon :name="$iconMap[$type] ?? 'arrow-turn-down-right'" variant="micro" />
            ...
            @if($results)
                <flux:command.items>
                    @foreach($results as $result)
                        <flux:command.item class="!h-auto flex items-center gap-x-1" wire:click="processIntent({{ json_encode($result) }})" wire:key="{{ $result->type . '_' . $result->value }}">
                            @if(array_key_exists($result->type, $iconMap))
                                <flux:icon :icon="$iconMap[$result->type]" variant="micro" />
                            @endif
                            <span>
                                {{ $result->label }}
                            </span>
