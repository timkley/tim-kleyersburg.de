<flux:modal name="parent-search" class="space-y-4 w-[calc(100vw-var(--spacing)*10)]">
    ...
    <flux:select autofocus x-on:change="$dispatch('select', $event.target.value)" variant="combobox" :filter="false">
        <x-slot name="input">
            <flux:select.input wire:model.live.debounce="searchTerm"/>
        </x-slot>
