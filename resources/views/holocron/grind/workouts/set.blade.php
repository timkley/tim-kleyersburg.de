...
>
    ...
    <flux:input.group>
        @if($set->finished_at && $set->reps < $minReps)
            <flux:input.group.prefix>
                <flux:icon variant="micro" icon="arrow-down" />
            </flux:input.group.prefix>
        @endif
        <flux:input class:input="text-center font-bold" wire:model.live="weight"/>
            @if($set->finished_at && $set->reps > $maxReps)
                <flux:input.group.suffix>
                    <flux:icon variant="micro" icon="arrow-up" />
                </flux:input.group.suffix>
            @endif
    </flux:input.group>
    <flux:input.group>
        <flux:input.group.prefix x-on:click="$wire.reps--; $wire.commit()">-</flux:input.group.prefix>
        <flux:input class:input="text-center font-bold" wire:model.live="reps"/>
    ...
    @endif
    @if($set->started_at && $set->finished_at)
        <flux:button icon="check-circle" variant="subtle" class="!text-green-800"></flux:button>
    @endif
</div>
