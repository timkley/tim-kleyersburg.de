...
class NotesService
{
    ...
    public function write(string $path, string $content): void
    {
        ...
        if ($real === false) {
            ...
            $real = realpath($parentDir);
        }

        if ($real === false || ! str_starts_with($real, $this->basePath)) {
            throw new RuntimeException("Path escapes base directory: {$path}");
        }

        file_put_contents($real.'/'.basename($target), $content);
    }
    ...
    public function search(string $query, int $limit = 10): array
    {
        $result = Process::path($this->basePath)->run([
            'grep', '-r', '-i', '-n', '--include=*.md', '-m', (string) $limit, $query, '.',
        ]);
